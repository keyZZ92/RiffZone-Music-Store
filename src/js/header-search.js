...
function initCatalogSearch() {
  ...
  if (searchInput && filters) {
    ...
    filters.addEventListener("focusout", () => {
      setTimeout(() => {
        if (
          !filters.contains(document.activeElement) &&
...
function applyFilters() {
  ...
  const offer = document.getElementById("offer-filter")?.value || "";
  const minPrice =
    parseFloat(document.getElementById("min-price")?.value || 0) || 0;
  const maxPrice =
  ...
  filteredProducts = products.filter((p) => {
    ...
    const matchesOffer =
      offer !== "on-offer" || (p.offerPrice && p.offerPrice < p.price);
  ...
  switch (sort) {
    case "price-asc":
      filteredProducts.sort((a, b) => {
        const priceA =
          a.offerPrice && a.offerPrice > 0 && a.offerPrice < a.price
            ? a.offerPrice
            : a.price;
        const priceB =
          b.offerPrice && b.offerPrice > 0 && b.offerPrice < b.price
            ? b.offerPrice
            : b.price;
...
function renderProducts(products) {
  ...
  if (!container) return;
  container.className = "product-list"; // Aplica grid universal
  container.innerHTML = "";
  ...
  products.forEach((product) => {
    ...
    card.innerHTML = `
      <div class="card h-100 guitar-card product-card" tabindex="0" role="button" aria-label="Ver detalles de ${
        product.name
      }">
          ...
          <p class="card-text">
            ${
              product.offerPrice && product.offerPrice < product.price
                ? `<span class='text-muted text-decoration-line-through'>${product.price}€</span> <span class='price-offer fw-bold text-danger'>${product.offerPrice}€</span>`
                : `<span class='price-offer fw-bold'>${product.price}€</span>`
            }
          </p>
          <div class="d-flex gap-2 mt-auto">
            <button class="btn btn-outline-secondary btn-detail flex-fill" type="button" aria-label="Ver detalle de ${
              product.name
            }">
